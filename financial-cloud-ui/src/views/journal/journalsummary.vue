<template>
  <div class="app-container">
    <el-card class="common-card">
      ...
      >
        <el-form-item
          :label="$t('jbx.journalsummary.yearPeriod')"
          prop="yearPeriodPicker"
        >
          <el-date-picker
            v-model="queryParams.yearPeriodPicker"
            placeholder=""
            type="month"
    ...
    <el-card class="common-card">
      ...
      >
        ...
        <el-table-column
          :label="$t('jbx.journalsummary.accName')"
          align="left"
          prop="accName"
        />
        <el-table-column
          :label="$t('jbx.journalsummary.openingBalance')"
          align="right"
          prop="openingBalance"
        >
          <template #default="scope">
        ...
        <el-table-column
          :label="$t('jbx.journalsummary.income')"
          align="right"
          prop="income"
        >
          <template #default="scope">
        ...
        <el-table-column
          :label="$t('jbx.journalsummary.expenditure')"
          align="right"
          prop="expenditure"
        >
          <template #default="scope">
        ...
        <el-table-column
          :label="$t('jbx.journalsummary.closingBalance')"
          align="right"
          prop="closingBalance"
        >
          <template #default="scope">
        ...
        <el-table-column
          :label="$t('jbx.journalsummary.yearIncome')"
          align="right"
          prop="yearIncome"
        >
          <template #default="scope">
        ...
        <el-table-column
          :label="$t('jbx.journalsummary.yearExpenditure')"
          align="right"
          prop="yearExpenditure"
        >
          <template #default="scope">
...
function getList(): any {
  ...
  journalSummaryService.fetch(queryParams.value).then((res: any) => {
    ...
    if (res.code === 0) {
      ...
      tableSummary.value = res.data.tableSummary;
    }
  });
}
...
const getSummaries = () => {
  ...
  sums[2] = ''
  sums[3] = ''
  sums[4] = formatAmount(tableSummary.value.openingBalance);
  sums[5] = formatAmount(tableSummary.value.income);
  sums[6] = formatAmount(tableSummary.value.expenditure);
  sums[7] = formatAmount(tableSummary.value.closingBalance);
  sums[8] = formatAmount(tableSummary.value.yearIncome);
  sums[9] = formatAmount(tableSummary.value.yearExpenditure);
