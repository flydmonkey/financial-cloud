...
public class RedisSessionManager implements SessionManager {
	...
	public static final String PREFIX = "jb:session:%s";

	public static final String PREFIX_TWOFACTOR = "jb:session:twofactor:%s";
	...
	//存储当前用户的会话
	public static final String USER_SESSION 		= 	"jb:session:user:%s";

	//租户端每个用户最多登录5个浏览器
	public static final String WEB_SESSION 			=	"jb:session:web:%s:%s";

	//平台关联端会话和租户端一致
	public static final String MGMT_SESSION 		=	"jb:session:mgmt:%s:%s";

	//APP端会话只有一个
	public static final String APP_SESSION 			=	"jb:session:app:%s";
	 ...
	 */
	public RedisSessionManager(StringRedisTemplate redis, JsonMapper jsonMapper, int validitySeconds) {
		this.redis = redis;
	...
	public void create(String sessionId, Session session) {
		...
		try {
			redis.opsForValue().set(formatKey(sessionId), jsonMapper.writeValueAsString(session),
					Duration.ofSeconds(validitySeconds));
		} catch (Exception e) {
			throw new IllegalStateException("redis session create failed", e);
		}
	}

	@Override
	public Session remove(String sessionId) {
		Session ticket = get(sessionId);
    ...
    public Session get(String sessionId) {
		String json = redis.opsForValue().get(formatKey(sessionId));
		if (json == null) {
			return null;
		}
		try {
			return jsonMapper.readValue(json, Session.class);
		} catch (Exception e) {
			throw new IllegalStateException("redis session read failed", e);
		}
    }
